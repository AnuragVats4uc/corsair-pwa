...
const Graph: React.FC<GraphProps> = ({
    ...
    const getGraphCellSingleColumn = (data: ChartBar[]) => {
    ...
    const getGraphVerticalSingleBar = (data: ChartBar[]) => (
        <BarChart
            layout={layout}
        ...
        >
            ...
            <Bar dataKey={DEFAULT_GRAPH_CONFIG.dataKeyBar} barSize={barSize}>
                ...
                {getGraphCellSingleColumn(data as ChartBar[])}
            </Bar>
        </BarChart>
    )

    const getGraphHorizontalSingleBar = (data: ChartBar[]) => {
        return (
            <BarChart
                data={data}
                layout={GraphLayout.HORIZONTAL}
    ...
    const getGraphVerticalDoubleBar = (data: ChartBar[]) => {
        const colorFirstColumn = data[0].firstColumnFill
        const colorSecondColumn = data[0].secondColumnFill
        const legendFirstColumn = data[0].firstBarLabel
        const legendSecondColumn = data[0].secondBarLabel
    ...
    const getGraph = () => {
        if (layout === GraphLayout.VERTICAL && barFormat === BarFormat.SINGLE)
            return getGraphVerticalSingleBar(graphData as ChartBar[])
        if (layout === GraphLayout.HORIZONTAL && barFormat === BarFormat.SINGLE)
            return getGraphHorizontalSingleBar(graphData as ChartBar[])
        if (layout === GraphLayout.VERTICAL && barFormat === BarFormat.DOUBLE)
            return getGraphVerticalDoubleBar(graphData as ChartBar[])
        return <span />
    }
